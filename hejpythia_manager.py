...
def make_job_file(user_name, job_number, events, threads, base_dir, rivet_dir, output_dir, name):
    """
    ...
    cmd += """(arguments = '-u' '%s' '-j' '%s' '-t' '%s' '-e' '%s' '-b' '%s' '-r' '%s' '-o' '%s')\n""" % (user_name, job_number, threads, events, base_dir, rivet_dir, output_dir)
    cmd += """(jobname = %s.%s)\n""" % (name, job_number)
    cmd += """(stdout = 'stdout')\n(stderr = 'stderr')\n(gmlog = 'job%s.log')\n""" % (job_number)
    cmd += """(count = '%s')\n(countpernode = '%s')" """ % (threads, threads)
...
def run(args):
    ...
    for idx in range(args["n_min"], args["n_max"] + 1):
        make_job_file(args["user_name"], idx, args["events"], args["threads"],
...
def main(args):
    ...
    """
    parser = argparse.ArgumentParser(description = "Usage: python hejpythia-manager.py -r [--run] -s [-status] -f [--finalise] -m [--merge]")
